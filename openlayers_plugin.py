...
class OpenlayersPlugin:
  ...
  def __init__(self, iface):
    ...
    self.iface = iface
	#load frensh translation
    oSetting = QSettings()
    if oSetting.value("locale/overrideFlag").toString() == "true":
        self.slocaleFullName = oSetting.value("locale/userLocale").toString()[0:2]
    else:
        self.slocaleFullName = QLocale().system().name()[0:2]
    sOpenLayersPath =":/plugins/i18n/fr_FR.qm"
    self.otranslator = QTranslator()
    if self.slocaleFullName == "fr":
        if self.otranslator.load( sOpenLayersPath):
            QCoreApplication.installTranslator( self.otranslator )
    # Layers
  ...
  def initGui(self):
    ...
    for layerType in self.olLayerTypeRegistry.types():
      ...
      action = QAction(QIcon(pathPlugin % layerType.icon), QApplication.translate("OpenLayersPlugins", "Add", None, QApplication.UnicodeUTF8) + " %s "%layerType.name + QApplication.translate("OpenLayersPlugins", "layer", None, QApplication.UnicodeUTF8), self.iface.mainWindow())
    ...
    if not self.__setCoordRSGoogle():
      QMessageBox.critical(self.iface.mainWindow(), "OpenLayers Plugin", QApplication.translate("OpenLayersPlugins", "Could not set Google projection!", None, QApplication.UnicodeUTF8))
      return
